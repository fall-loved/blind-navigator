...
const baseMap: BuildingMap = {
    ...
    'entrance': {
        ...
        edges: [{ to: 'lobby_1', instruction: 'Пройдите прямо в фойе к лестнице.', distance: 10 }]
    },
    'lobby_1': {
        id: 'lobby_1', name: 'Фойе 1 этажа', floor: 1,
        description: 'Вы в главном фойе 1 этажа.',
        edges: [
            { to: 'entrance', instruction: 'Идите на выход из здания.', distance: 10 },
            { to: 'stairs_1', instruction: 'Подойдите к центральной лестнице.', distance: 5 }
        ]
    },
    'stairs_1': {
        ...
        edges: [
            { to: 'lobby_1', instruction: 'Вернитесь в фойе.', distance: 5 },
            { to: 'stairs_2', instruction: 'Поднимитесь на 2 этаж.', distance: 12 }
        ]
    },
    'stairs_2': {
        ...
        edges: [
            { to: 'stairs_1', instruction: 'Спуститесь на 1 этаж.', distance: 12 },
            { to: 'stairs_3', instruction: 'Поднимитесь на 3 этаж.', distance: 12 }
        ]
    },
    'stairs_3': {
        ...
        edges: [
            { to: 'stairs_2', instruction: 'Спуститесь на 2 этаж.', distance: 12 }
        ]
    },
};
...
function buildWing(floor: number, startNodeId: string, segments: SegmentDef[]): BuildingMap {
    ...
    const forwardBearing = 0;
    const backwardBearing = 180;
    const roomBearing = 90;

    segments.forEach((seg, index) => {
        map[seg.segmentId] = {
            id: seg.segmentId, name: seg.name, floor: floor as 1 | 2 | 3,
            description: `Вы в коридоре: ${seg.name}.`, edges: []
        };

        map[seg.segmentId].edges!.push({
            to: prevNodeId, instruction: 'пройдите в предыдущую часть.',
            bearing: backwardBearing,
            distance: 10
        });

        if (index === 0) {
            baseMap[startNodeId].edges!.push({
                to: seg.segmentId, instruction: `пройдите в ${seg.name}.`,
                bearing: forwardBearing,
                distance: 10
            });
        } else {
            map[prevNodeId].edges!.push({
                to: seg.segmentId, instruction: 'пройдите в следующую часть.',
                bearing: forwardBearing,
                distance: 10
            });
        }
        ...
        seg.rooms.forEach(roomNum => {
            ...
            map[roomId] = {
                id: roomId, name: `Аудитория ${roomNum}`, floor: floor as 1 | 2 | 3,
                description: `Вы у аудитории ${roomNum}.`,
                edges: [{ to: seg.segmentId, instruction: 'выйдите в коридор.', bearing: 270, distance: 2 }]
            };

            map[seg.segmentId].edges!.push({
                to: roomId, instruction: `дверь аудитории ${roomNum}.`,
                bearing: roomBearing,
                distance: 2
            });
        });
